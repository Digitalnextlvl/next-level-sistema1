...
}) => <div className="rounded-2xl border border-border bg-foreground/5 backdrop-blur-sm transition-colors focus-within:border-foreground/70 focus-within:bg-foreground/10">
    {children}
  </div>;
...
export const SignInPage: React.FC<SignInPageProps> = ({
  ...
  return <div className="h-[100dvh] flex flex-col md:flex-row font-geist w-[100dvw]">
        ...
        <div className="w-full max-w-md">
          <div className="flex flex-col gap-6">
            ...
            <form className="space-y-5" onSubmit={onSignIn}>
              {mode === 'signup' && <div className="animate-element animate-delay-300">
                  <label className="text-sm font-medium text-muted-foreground">Nome Completo</label>
                  <GlassInputWrapper>
                    <input name="name" type="text" placeholder="Digite seu nome completo" className="w-full bg-transparent text-sm p-4 rounded-2xl focus:outline-none" required />
                  </GlassInputWrapper>
                </div>}
              ...
              {mode !== 'reset' && <div className="animate-element animate-delay-400">
              ...
              {mode === 'signin' && <div className="animate-element animate-delay-500 flex items-center justify-between text-sm">
                ...
                e.preventDefault();
                onResetPassword?.();
              }} className="hover:underline text-foreground transition-colors">Redefinir senha</a>
                </div>}

              <button type="submit" disabled={isSubmitting} className="animate-element animate-delay-600 w-full rounded-2xl bg-primary py-4 font-medium text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                {isSubmitting ? <div className="flex items-center justify-center gap-2">
                  ...
                  </div> : mode === 'signin' ? 'Entrar' : mode === 'signup' ? 'Criar Conta' : 'Redefinir Senha'}
              </button>
            </form>
      ...
      {heroImageSrc && <section className="hidden md:block flex-1 relative p-4">
          <div className="animate-slide-right animate-delay-300 absolute inset-4 rounded-3xl bg-cover bg-center" style={{
